...
6. 设置时长、分辨率、比例和高级参数。
7. 如需本地图片参考，展开 `TOS 图片上传` 并配置 TOS。
8. 在中间区域选择 `纯文本`、`参考图` 或 `多模态`。
9. 填写提示词；参考图可以直接选择本地图片上传，也可以手动填写公网 URL。
10. Prompt 中可写 `图片1`、`图片2` 引用对应图片槽位。
11. 点击 `生成视频`。
12. 页面会自动提交任务、轮询状态，并在成功后下载 MP4 到本机。
13. 生成成功后可在右侧预览、下载，也可以从历史记录重新打开。
...
Ark 的 `image_url` 必须能被云端访问。本工具支持把本地图片先上传到火山 TOS，再自动把 URL 填入参考图槽位。
...
- `对象前缀`，默认 `seedance-references`
- `URL 模式`，默认 `预签名 URL`

推荐使用 `预签名 URL`：桶可以保持私有，工具会生成默认 24 小时有效的访问链接给 Ark 读取。也可以切到 `公开 URL`，此时对象会尝试以 `public-read` ACL 上传；如果你使用 CDN 或自定义域名，可以填写 `公开 Base URL`。

TOS 配置文件也支持 `.txt`、`.env`、`.json`：
...
点击图片槽位里的 `上传` 后，工具会把本地图片上传到 TOS，并自动把返回 URL 填入对应槽位。Prompt 文本框可以通过 `图片1`、`图片2`、`图片3`、`图片4` 引用这些图片。
