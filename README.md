...
5. 可选点击 `+ 上传素材`，图片、参考视频、参考音频会先上传到 TOS，再进入资源库。
6. Prompt 里使用 `@` 引用资源库素材，例如 `@图片1`、`@视频1`、`@音频1`；只有被 @ 到的素材会进入本次生成请求。
7. 点击 `生成视频`。
...
Ark 需要读取公网可访问的素材 URL。本工具支持通过一个 `+ 上传素材` 入口把本地图片、视频、音频上传到火山 TOS，再放入页面资源库。
...
- 上传视频时，后端会先用 `ffprobe` 检测帧率、分辨率和时长。
- 如果参考视频帧率大于 `60fps`，会自动用 `ffmpeg` 转成 `30fps` 后再上传到 TOS。
- 资源库会显示检测结果，例如 `30fps · 1920x1080 · 5s · 已转 30fps`。
- 如果部署机器没有安装 FFmpeg/ffprobe，视频上传会提示先安装或手动转码。
...
- 在 Prompt 输入框输入 `@` 会弹出可引用素材列表。
- 备选列表会显示资源预览：图片缩略图、视频预览、音频类型卡片和文件名。
- 点击 `@图片1`、`@视频1`、`@音频1` 会把引用插入 Prompt。
- 生成时只提交 Prompt 中实际出现的 @ 素材。
- 如果填写了素材 URL 但 Prompt 没有 @ 引用，系统会提示补充引用，避免误传素材。
...
```text
首帧为 @图片1，保持 @图片1 中的同一位亚洲女性医生形象、白大褂、医美诊室暖光风格。参考 @视频1 的镜头运动节奏，音色参考 @音频1。
```

做多段长视频时，可以开启 `返回尾帧`，把上一段尾帧作为下一段的 `图片1` 上传，并在 Prompt 中写 `@图片1` 引用。
